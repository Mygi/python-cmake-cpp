...
namespace cvnp
{
    ...
    namespace detail
    {
        ...
        {
            for (auto format_synonym : cvnp::sTypeSynonyms) {
                std::cout << format_synonym.np_format << " vs " << dt.kind() << dt.itemsize() << std::endl;  
                int size = (int)dt.itemsize();                
                if (format_synonym.np_kind[0] == dt.kind() && size == format_synonym.num_bytes)
